...
class DB {
    ...
    viewEmployeesDepartment(choice){
        return this.connection.promise().query('SELECT employee.id AS employee_id, CONCAT(first_name, " ", last_name) AS name FROM employee LEFT JOIN role ON role_id= role.id LEFT JOIN department ON department_id=department.id WHERE department.id = ?', choice);
    }
